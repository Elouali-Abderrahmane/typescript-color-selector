let allList = document.querySelectorAll("li") as NodeListOf<HTMLLIElement>;
...
if (window.localStorage.color) {
  ...
  if (
    window.localStorage.color === "blue" ||
    window.localStorage.color === "black"
  ) {
    console.log(window.localStorage.color);
    result.style.color = "white";
    result.innerHTML = `${window.localStorage.color[0].toUpperCase()}${window.localStorage.color.slice(
      1
    )}`;
  } else {
